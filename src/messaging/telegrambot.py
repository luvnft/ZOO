...
class TelegramBot(BaseMessaging):
    ...
    async def recieve_message(cls, request: Request) -> Message:
        ...
        try:
            ...
            message_data = TelegramMessage(**request_body)
        except ValidationError as e:
            print(f"Error parsing message data: {e}")
            raise MessageParsingError("Invalid message format.") from e

        message_type = MessageType.TEXT
        if isinstance(
            message_data.message, (_TelegramMessagePhoto, _TelegramMessageDocument)
        ):
            message_type = MessageType.FILE

        user_name = (
            f"{message_data.message.from_.first_name} "
            f"{message_data.message.from_.last_name}"
        )

        return Message(
            message=message_data,
            uid=message_data.message.from_.id,
            user_name=user_name,
    ...
    async def send_message(self, message: str, user_id: int) -> Optional[str]:
        ...
        try:
            sent_message = await self._bot.send_message(chat_id=user_id, text=message)
