...
class ZootopiaContext:
    def __init__(self, request_body, config: ZootopiaConfig):
        ...
        """
        self.database: SupabaseDB = SupabaseDB.from_config(config.DATABASE_CONFIG.SUPABASE)
        self.messaging_service: MessageProviderBase = self._create_messaging_provider(request_body, config)
        ...
        self.room: RoomModel = self._get_or_create_room_from_db(self.message, self.user, self.agent)

    def _create_messaging_provider(self, request_body, config: ZootopiaConfig) -> MessageProviderBase:
    ...
    def _get_or_create_user_from_db(self, message: ZootopiaMessage) -> UserModel:
        ...
        if not user:
            new_user = UserModel(
                telegram_uid=message.metadata.uid if isinstance(message.metadata, TelegramMetadata) else None,
                phone_number=message.metadata.phone_number if isinstance(message.metadata, BirdMetadata) else None
            )
    ...
    def _get_or_create_agent_from_db(self, message: ZootopiaMessage) -> AgentModel:
        ...
        if not agent:
            new_agent = AgentModel(
                telegram_chat_id=message.metadata.chat_id if isinstance(message.metadata, TelegramMetadata) else None,
                bird_channel_id=message.metadata.channel_id if isinstance(message.metadata, BirdMetadata) else None
            )
    ...
    def _get_or_create_room_from_db(self, message: ZootopiaMessage, user: UserModel, agent: AgentModel) -> RoomModel:
